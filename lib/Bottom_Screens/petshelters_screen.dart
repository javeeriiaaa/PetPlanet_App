...
class PetSheltersScreen extends StatefulWidget {
  const PetSheltersScreen({Key? key}) : super(key: key);

  @override
  State<PetSheltersScreen> createState() => _PetSheltersScreenState();
}

Future<DocumentSnapshot<Map<String, dynamic>>> _getPetData() async {
  final petsCollection = FirebaseFirestore.instance.collection('pets');
  final petDocument = await petsCollection.doc('pet').get();
  return petDocument;
}

class _PetSheltersScreenState extends State<PetSheltersScreen> {
  _callNumber(String number) async {
    await FlutterPhoneDirectCaller.callNumber(number);
  }

  List<String> categories = ['Cats', 'Dogs', 'Birds', 'Fish'];
                  ...
                  cats.length,
                    ...
                    child: GestureDetector(
                        onTap: () => _callNumber('0349 7405097'),
                          ...
                          //     MaterialPageRoute(
                          //         builder: (context) =>
                          //            SheltersDetailPage(cat: cats[index])));
                        child: CatItem(shelter: shelter[index])),
                  ),
                ),
              ),
            )
  ...
  Widget build(BuildContext context) {
    return ClipRRect(
      ...
      child: Container(
        ...
        color: shelter.color.withOpacity(0.6),
        child: Stack(
          children: [
            Positioned(
              bottom: -10,
                  ...
                  Container(
                    ...
                    child: Icon(
                      shelter.fav
                          ...
                          : Icons.favorite_outline_rounded,
                      color: shelter.fav ? red : black.withOpacity(0.6),
                    ),
                  )
                ],
              ),
